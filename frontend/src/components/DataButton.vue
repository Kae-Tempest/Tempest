<template>
    <button class="bg-ctp-surface0 hover:bg-ctp-surface1 text-ctp-blue py-2 px-4 rounded border border-white" @click="ShowData()">
        Last 24h report
    </button>
    ...
    <div class="relative w-full h-screen max-w-7xl">
        ...
        <div class="relative rounded-lg shadow-lg bg-ctp-base border border-ctp-peach">
            ...
            <div class="flex items-center p-5 border-b rounded-t dark:border-gray-600">
                <h3 class="text-xl text-ctp-text ml-[40%]">
                   Last 24h Reports
                </h3>
...
export default {
    ...
    methods: {
        ShowData: async function () {
            const res = await axios.get(`http://127.0.0.1:5000/report/${this.id}`)
            ...
            reports.forEach(element => {
                ...
                this.LastReports = LastDayReports.reverse()
            });
