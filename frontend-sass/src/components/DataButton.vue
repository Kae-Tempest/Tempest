...
const ShowData = async function () {
    const {data : LastDayReports} = await axios.get(`http://192.168.1.28:5000/dayreport/${props.id}`)
    LastReports = LastDayReports.rows
    console.log(LastReports.length)
    LastReports.forEach(element => {
      let date = new Date(Number(element.mesured_at))
      element.mesured_at = `${date.getDate().toString().padStart(2, '0')} | ${date.getHours().toString().padStart(2, '0')}h${date.getMinutes().toString().padStart(2, '0')}`;
    });
